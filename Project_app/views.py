...
from accounts import views as auth_v
...
def form(request):
    if 'username' in request.session:
        if request.method=="POST":
            ...
            A.save()
            P = Publisher(publisher_name=publisher_name, country=country)
            P.save()
            B = Book(title=title,genre=genre,publisher=P)
            B.save()
            B.author.add(A) #ManyToMany field query
            B.save()
...
def table(request):
    if "username" in request.session:
        ...
        publisher = Book.objects.values_list('publisher_id', flat=True)
        publisher_id = list(publisher)[0]
        context['Book'] = Book.objects.filter(author=author).values()
        context['Publisher'] = Publisher.objects.all()
        context['Author'] = Author.objects.filter(user=username)
        context['Publisher'] = Publisher.objects.filter(id=publisher_id)
